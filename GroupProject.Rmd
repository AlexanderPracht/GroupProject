...
main_data4 <- main_data3 %>%
  mutate(room_type = factor(room_type, order = TRUE, levels = c("Entire home/apt", "Hotel room", "Private room", "Shared room")),
...
Best <-c("Central & Western","Kowloon City","Islands")
Better <-c("Southern","Tsuen Wan","Wan Chai")
Good <-c("Sai Kung", "Tai Po", "Tuen Mun")
Standard <-c("Sham Shui Po","Kwai Tsing", "Kwun Tong")
Worse <- c("Sha Tin", "Eastern","Yau Tsim Mong")
Worst <- c("Yuen Long", "Wong Tai Sin","North")
...
model10 <- lm(log(price_4_nights) ~ neighbourhood_simplified + is_location_exact + cancellation_policy +  host_is_superhost + host_since_calculated + security_deposit + bedrooms + accommodates + total_amenities + prop_type_simplified + number_of_reviews + number_of_reviews_ltm + review_scores_rating + room_type, data= main_data4)
