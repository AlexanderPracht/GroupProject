...
```{r, price_4_nights vs room_type, fig.width=12, fig.height= 8}
ggplot(main_data4, aes(y= log(price_4_nights), x=room_type))+
  ...
  theme_minimal()
```
So we see that Private Room has the lowest median price, followed by Hotel Room and Entire Home/Apartment. Shared room seems to have a quite high median price for 4 nights. This seems odd. Let us dubble check:

```{r, plot_avg_price_room_type}
...
```{r, price_4_nights vs neighbourhood_simplified, fig3, fig.width=12, fig.height= 8}
...
```{r, price_4_nights vs is_it_big, fig4, fig.width=12, fig.height= 8}
...
```{r, warning=FALSE, fig7,  fig.width=20, fig.height=20}
main_data4%>%
...
```{r, price_4_nights and number_of_reviews, fig8, fig.width=12, fig.height= 8}
...
```{r, price_4_nights and number_of_reviews_ltm, fig9, fig.width=12, fig.height= 8}
#Scatterplot of price_4_nights vs number of reviews
...
```{r, price_4_nights vs Host Experience, fig10, fig.width=12, fig.height= 8}
...
Seems like host experience is going to help us a lot later in modelling...
